...
class PrevisionModel
{
     ...
     */
    public static function getAllPrevision()
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "SELECT user_id, prevision_id, prevision_text FROM prevision WHERE user_id = :user_id";
    ...
    public static function getPrevision($prevision_id)
    {
        ...
        $sql = "SELECT user_id, prevision_id, prevision_text FROM prevision WHERE user_id = :user_id AND prevision_id = :prevision_id LIMIT 1";
        $query = $database->prepare($sql);
    ...
    public static function createPrevision($prevision_text)
    {
        ...
        $sql = "INSERT INTO prevision (prevision_text, user_id) VALUES (:prevision_text, :user_id)";
        $query = $database->prepare($sql);
    ...
    public static function updatePrevision($prevision_id, $prevision_text)
    {
        ...
        $sql = "UPDATE prevision SET prevision_text = :prevision_text WHERE prevision_id = :prevision_id AND user_id = :user_id LIMIT 1";
        $query = $database->prepare($sql);
    ...
    public static function deletePrevision($prevision_id)
    {
        ...
        $sql = "DELETE FROM prevision WHERE prevision_id = :prevision_id AND user_id = :user_id LIMIT 1";
        $query = $database->prepare($sql);
